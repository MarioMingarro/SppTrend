...
#'
#' @param spp_trends_world_results A data frame containing trend indicators per species,
...
#'   column of 'spp_trends_world_results' will have corresponding classification columns
#'   in the output.
#'
...
#' @examples
#' \dontrun{
#' spp_trends_results <- data.frame(#'   species = paste0("spp_", 1:10),
#'   responses = rep(c("Lat", "Lon", "Tmx", "Tmn"), each = 2, length.out = 10),
...
spp_strategy_poleward <- function(spp_trend_world_result, bonferroni = 0.05, responses = responses) {
  ...
  strategies <- spp_trends_world_results %>%
    dplyr::select(dplyr::all_of(intersect(
      required_cols, names(spp_trends_world_results)
    ))) %>%
    dplyr::mutate(
